...
const screenshots = [
  tn1.url, tn2.url, tn3.url, tn4.url, tn5.url, 
  tn6.url, tn7.url, tn8.url, tn9.url, tn10.url,
...
const PhoneFrame = ({ src }: { src: string }) => (
  <div className="relative mx-auto w-full max-w-[320px] sm:max-w-[400px]">
    <div className="rounded-2xl border border-white/10 bg-card/30 p-2 shadow-2xl backdrop-blur-sm transition-all duration-300">
      <div className="relative overflow-hidden rounded-xl bg-black/20">
        <img 
          src={src} 
          alt="Depoimento de aluno" 
          className="w-full h-auto max-h-[80vh] object-contain block mx-auto transition-all duration-300" 
          loading="eager" 
        />
      </div>
    </div>
...
const useDragCarousel = (length: number, setIdx: (fn: (p: number) => number) => void) => {
  ...
  const onPointerUp = useCallback((e: React.PointerEvent) => {
    ...
    if (Math.abs(diff) > 50) {
      if (diff < 0) setIdx((p) => (p + 1) % length);
      else setIdx((p) => (p - 1 + length) % length);
    }
  ...
  return (
    <SectionWrapper id="depoimentos" className="py-20 bg-radial-top">
      <div className="mx-auto max-w-6xl px-4">
        ...
        <h2 className="mb-3 text-center font-display text-2xl font-bold sm:text-4xl leading-tight">
          Veja quem já está aprendendo<br className="hidden sm:inline" /> e <span className="text-gradient">faturando:</span>
        </h2>
        <p className="mb-12 text-center text-muted-foreground text-sm sm:text-base whitespace-nowrap">
          +1.000 alunos já transformaram suas vidas!
        </p>

        {/* Screenshot carousel - one at a time, drag support */}
        <div
        ...
        >
          <div className="flex items-center justify-center overflow-hidden py-4">
            <AnimatePresence mode="wait">
              ...
              >
                <PhoneFrame src={screenshots[imgIdx]} />
              </motion.div>
            </AnimatePresence>
          </div>

          <button
            onClick={() => setImgIdx((p) => (p - 1 + screenshots.length) % screenshots.length)}
            className="absolute left-2 sm:left-8 top-1/2 -translate-y-1/2 z-20 flex h-10 w-10 items-center justify-center rounded-full bg-card border border-border text-foreground hover:bg-primary/20 transition-colors"
          >
            <ChevronLeft className="h-5 w-5" />
          </button>
          <button
            onClick={() => setImgIdx((p) => (p + 1) % screenshots.length)}
            className="absolute right-2 sm:right-8 top-1/2 -translate-y-1/2 z-20 flex h-10 w-10 items-center justify-center rounded-full bg-card border border-border text-foreground hover:bg-primary/20 transition-colors"
          >
          ...
          <div className="flex justify-center gap-1.5 mt-4">
            {screenshots.map((_, i) => (
              <button
                key={i}
                onClick={() => setImgIdx(i)}
                className={`h-2 rounded-full transition-all ${i === imgIdx ? "w-6 bg-primary" : "w-2 bg-muted"}`}
              />
            ))}
          </div>
        </div>

        {/* Text testimonials carousel - 1 on mobile, 3 on desktop */}
        <div
        ...
        >
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="block sm:hidden">
                {(() => {
                  const t = testimonials[txtIdx % testimonials.length];
                  return (
                    <div className="card-hover rounded-xl border border-border bg-card p-6">
                      ...
                      <p className="mb-4 text-base text-muted-foreground italic">"{t.text}"</p>
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/20 font-display font-bold text-primary">
                          {t.name[0]}
                        </div>
                        <div>
              ...
              <div className="hidden sm:grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {[0, 1, 2].map((offset) => {
                  const t = testimonials[(txtIdx + offset) % testimonials.length];
                  return (
          ...
          <div className="flex justify-center gap-3 mt-6">
            <button
              onClick={() => setTxtIdx((p) => (p - 1 + testimonials.length) % testimonials.length)}
              className="flex h-10 w-10 items-center justify-center rounded-full bg-card border border-border text-foreground hover:bg-primary/20 transition-colors"
            >
              <ChevronLeft className="h-5 w-5" />
            </button>
            <button
              onClick={() => setTxtIdx((p) => (p + 1) % testimonials.length)}
              className="flex h-10 w-10 items-center justify-center rounded-full bg-card border border-border text-foreground hover:bg-primary/20 transition-colors"
            >
              <ChevronRight className="h-5 w-5" />
            </button>
        ...
        <div className="mt-10 flex justify-center">
          <CtaButton className="whitespace-nowrap">QUERO COMEÇAR AGORA!</CtaButton>
        </div>
      </div>
    </SectionWrapper>
  );
