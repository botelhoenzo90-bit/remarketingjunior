...
const HeroSection = () => (
  <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
    <div className="absolute inset-0 bg-background"><div className="absolute inset-0 opacity-[0.08]" style={{ backgroundImage: "linear-gradient(hsl(var(--primary)/0.4) 1px, transparent 1px), linear-gradient(90deg, hsl(var(--primary)/0.4) 1px, transparent 1px)", backgroundSize: "80px 80px" }} /><div className="absolute inset-0 bg-gradient-to-b from-transparent via-background/60 to-background" /></div>
    ...
    <div className="relative z-10 mx-auto max-w-5xl px-4 pt-6 pb-8 text-center">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.6, duration: 0.5 }} className="mx-auto mb-7 max-w-3xl overflow-hidden rounded-2xl border-2 border-primary shadow-[0_0_40px_hsl(192_95%_55%/0.4),0_0_80px_hsl(192_95%_55%/0.2)]"><div className="relative aspect-video"><img src={vslCover} alt="Capa VSL" className="absolute inset-0 h-full w-full object-cover" /><iframe src="https://www.youtube.com/embed/3ONlCDPSsEY?rel=0" title="VSL Mestre da Manutenção" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="absolute inset-0 h-full w-full" /></div></motion.div>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} className="mb-4"><CtaButton className="text-sm sm:text-lg px-6 sm:px-10 py-5 whitespace-nowrap">QUERO PARAR DE ADIAR →</CtaButton></motion.div>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="flex items-center justify-center gap-3 mb-5"><div className="flex -space-x-2">{[avatar1, avatar2, avatar3, avatar4].map((src, i) => <img key={i} src={src} alt="Aluno" className="h-8 w-8 rounded-full border-2 border-primary/60 object-cover" />)}</div><p className="text-xs sm:text-sm font-bold text-primary">+1.000 alunos formados</p></motion.div>
      <a href="#aulas-gratis" className="inline-flex items-center gap-2 text-xs font-semibold text-muted-foreground transition-colors hover:text-primary">Prefere conhecer antes de comprar? Veja as aulas gratuitas <ArrowDown className="h-4 w-4" /></a>
    </div>
